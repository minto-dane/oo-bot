...
- OO_CONFIG_PATH
- OO_CONTROL_SOCKET_PATH

補足:

- 未指定時は `config/oo-bot.yaml` を使います。
- `OO_CONFIG_PATH` は YAML ファイルの参照先を上書きします。
- `OO_CONTROL_SOCKET_PATH` は runtime control socket の配置先を明示したい場合だけ使います。
- 未指定時は `/run/oo-bot/control.sock`、`$XDG_RUNTIME_DIR/oo-bot/control.sock`、`/tmp` fallback の順で自動決定します。
...
export OO_CONTROL_SOCKET_PATH=/tmp/oo-bot-control.sock
